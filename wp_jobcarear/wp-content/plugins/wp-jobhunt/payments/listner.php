...
function cs_update_post($id = '', $cs_trans_id = '') {
	...
	if ($_REQUEST['payment_method'] == 'bacs') {
	    ...
	    do_action('jobhunt_job_status_changed', $id, $old_job_status); //job status approved/not approved email hook
	}else if(isset($cs_plugin_options['cs_jobs_review_option']) && $cs_plugin_options['cs_jobs_review_option'] != 'on' && $_REQUEST['payment_status'] == 'Completed'){
		$old_job_status = get_post_meta($id, 'cs_job_status', true); // getting job status
	    update_post_meta((int) $id, 'cs_job_status', 'active'); 
	    do_action('jobhunt_job_status_changed', $id, $old_job_status); 
	} else {
...
if (isset($_REQUEST['payment_status']) && $_REQUEST['payment_status'] == 'Completed') {
    ...
    $job_id = $_REQUEST['item_number'];
    if (isset($job_id) && $job_id != '') {
	if (isset($_REQUEST['txn_id']) && $_REQUEST['txn_id'] <> '') {

	    $transaction_array = array();

	    $transaction_array['cs_trans_id'] = esc_attr($_REQUEST['txn_id']);
	    $transaction_array['cs_job_id'] = esc_attr($_REQUEST['item_number']);
	    $transaction_array['cs_transaction_status'] = 'approved';
	    $transaction_array['cs_full_address4'] = esc_attr($_REQUEST['address_street']) . ' ' . esc_attr($_REQUEST['address_city']) . ' ' . esc_attr($_REQUEST['address_country']);
	    $transaction_array['cs_transaction_amount'] = esc_attr($_REQUEST['payment_gross']);
	    $transaction_array['cs_transaction_pay_method'] = 'cs_paypal_gateway';
	    $transaction_array['cs_trans_currency'] = esc_attr($_REQUEST['mc_currency']);
	    $transaction_array['cs_summary_email'] = esc_attr($_REQUEST['payer_email']);
	    $transaction_array['cs_first_name'] = esc_attr($_REQUEST['first_name']);

	    $transaction_array['cs_last_name'] = esc_attr($_REQUEST['cs_last_name']);

	    $cs_trans_id = isset($_REQUEST['custom']) ? $_REQUEST['custom'] : '';
...
if (isset($_REQUEST['x_response_code']) && $_REQUEST['x_response_code'] == '1') {
	...
	$transaction_array['order_id'] = esc_attr($_REQUEST['x_po_num']);

	$transaction_array['summary_status'] = 'Completed';
	$transaction_array['cs_trans_id'] = esc_attr($_REQUEST['x_trans_id']);
	$transaction_array['cs_transaction_amount'] = esc_attr($_REQUEST['x_amount']);
	$transaction_array['cs_trans_currency'] = 'USD';

	$transaction_array['address_street'] = esc_attr($_REQUEST['x_address']);
	$transaction_array['address_city'] = esc_attr($_REQUEST['x_city']);
	$transaction_array['address_country'] = esc_attr($_REQUEST['x_country']);
	$transaction_array['cs_full_address2'] = esc_attr($_REQUEST['x_address']) . ' ' . esc_attr($_REQUEST['x_city']) . ' ' . esc_attr($_REQUEST['x_country']);

	if (esc_attr($_REQUEST['x_email'] == '')) {
	    $transaction_array['cs_summary_email'] = cs_get_user_data($transaction_array['order_id'], 'email');
	} else {
	    $transaction_array['cs_summary_email'] = esc_attr($_REQUEST['x_email']);
	}

	if (esc_attr($_REQUEST['x_first_name'] == '')) {
	    $transaction_array['cs_first_name'] = cs_get_user_data($transaction_array['order_id'], 'first_name');
	} else {
	    $transaction_array['cs_first_name'] = esc_attr($_REQUEST['x_first_name']);
	}

	if (esc_attr($_REQUEST['x_last_name'] == '')) {
	    $transaction_array['cs_last_name'] = cs_get_user_data($transaction_array['order_id'], 'last_name');
	} else {
	    $transaction_array['cs_last_name'] = esc_attr($_REQUEST['x_last_name']);
	}

	$package_id = get_post_meta((int) $transaction_array['order_id'], 'transaction_package', true);

	$cs_trans_id = isset($_REQUEST['x_po_num']) ? $_REQUEST['x_po_num'] : '';
...
if (isset($_REQUEST['merchant_id'])) {
    ...
    $concatFields = $_REQUEST['merchant_id']
	    . $_REQUEST['order_id']
    ...
    if (isset($_REQUEST['status']) && $_REQUEST['status'] == '2' && trim($_REQUEST['pay_to_email']) == trim($MBEmail)) {
	$data = explode('||', $_REQUEST['transaction_id']);
	$order_id = $data[0];
	$job_id = $data[1];
	...
	if (isset($job_id) && $job_id != '') {
	    ...
	    $transaction_array['summary_status'] = 'Completed';
	    $transaction_array['cs_trans_id'] = esc_attr($_REQUEST['mb_transaction_id']);
	    $transaction_array['cs_transaction_amount'] = esc_attr($_REQUEST['amount']);
	    $transaction_array['cs_trans_currency'] = $_REQUEST['currency'];
